...
clock_t end;
double timing;
unsigned num_generated_nodes;
unsigned num_expanded_nodes;
unsigned num_max_depth;
FILE *out = NULL;
...
void ExitProgram(const char *message) {
	...
	fprintf(out, "Propagation = %s\n", p);
	fprintf(out, "Budget=%d\n", budget);
	fprintf(out, "MaxDepth = %d\n", num_max_depth);
	fprintf(out, "TotalGenerated = %d\n", num_generated_nodes);
	fprintf(out, "TotalExpanded = %d\n", num_expanded_nodes);
	fprintf(out, "Time = %f seconds\n", timing);
	fprintf(out, "Expanded/Second = %d\n", (int)(num_expanded_nodes/timing));
	fprintf(out, "Score = %d\n", Points);
